...
export default function DisplayCard({
  ...
  return (
    <>
      {tasks && (
        <div className="col-md-4 col-sm-1 p-5" key={id}>
              ...
              {techStack && techStack.length !== 0 ? (
                <p className="displayLang">
                  {techStack.map((tech, index) => (
                    <button className="btnLanguage" key={index}>
                      {" "}
                      {tech}
                    </button>
                  ))}
                ...
                <p className="displayLang">
                  <button className="btnLanguage">Open for all stacks</button>
                </p>
              )}
              <div className="displayLang">
                ...
                <div style={{ display: "flex", alignItems: "center" }}>
                  ...
                  <span style={{ paddingLeft: "0.5em" }}>
                    {new Date(createdAt).toString().substring(0, 10)}
                  </span>
                </div>
              </div>
              <hr></hr>
              {status === "Done" ? (
                <p className="displayStatus" style={{ color: "lightgray" }}>
                  DONE
                </p>
              ) : (
